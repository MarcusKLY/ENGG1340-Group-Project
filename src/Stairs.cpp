...
int Stairs(string player_name)
{
    ...
    char_typewriter("Suddenly, you recall a myth that is well known in the campus...", italic_green);
    ...
    this_thread::sleep_for(chrono::seconds(1));
    char_typewriter("You🙎: No way this is happening to me 😱", bold_magenta);
    char_typewriter("You feel like something is dragging your legs and it's getting heavier", italic_green);
    char_typewriter("Your sixth sense tells you not to look behind!", bold_red);
    ...
    if (look_back == 0)
    {
        ...
        char_typewriter("However, your feet are so heavy that you can barely make a step", italic_green);
        ...
        if (look_back_again == 0)
        {
            ...
            char_typewriter("You🙎: 😓😓😓", bold_magenta);
